...
intMap rm[] = {
    {BXandSI, "000"}, {BXandDI, "001"}, {BPandSI, "010"},    {BPandDI, "011"},
    {SI, "100"},      {DI, "101"},      {BPorDirect, "110"}, {BX, "111"},
...
bool isImmediate(String oprand) {
  return !isRegisterType(oprand) && !isMemoryType(oprand) &&
         !isSegmentRegister(oprand);
}
...
int setOperationType(Operand *Operand1, Operand *Operand2) {
  if (Operand1->type.OperandType == registerType &&
      Operand2->type.OperandType == registerType) {
    return regToReg;
  }
  if (Operand1->type.OperandType == registerType &&
      Operand2->type.OperandType == memoryType) {
    if (strcmp(Operand1->value, "AX") == 0 ||
        strcmp(Operand1->value, "AL") == 0) {
      return MemToAcc;
    }
    return memToReg;
  }
  if (Operand1->type.OperandType == memoryType &&
      Operand2->type.OperandType == registerType) {
    if (strcmp(Operand2->value, "AX") == 0 ||
        strcmp(Operand2->value, "AL") == 0) {
      return AccToMem;
    }
    return regToMem;
  }
  if (Operand2->type.OperandType == immediateType &&
      (Operand1->type.OperandType == registerType ||
       Operand1->type.OperandType == memoryType)) {
    if (Operand1->type.OperandType == registerType) {
      return ImmToReg;
    }
    return ImmToRegM;
  ...
  if ((Operand1->type.OperandType == memoryType ||
       Operand1->type.OperandType == registerType) &&
      Operand2->type.OperandType == segmentRegisterType) {
    return SregTorm;
  }
  if (Operand1->type.OperandType == segmentRegisterType &&
      (Operand2->type.OperandType == memoryType ||
       Operand2->type.OperandType == registerType)) {
    return rmToSreg;
  } else {
...
void setRegistersInvolvedInMemory(Operand *Operand) {
  if (Operand->type.OperandType == memoryType) {
    if (isSubStringExist("BX", Operand->value) &&
        isSubStringExist("SI", Operand->value)) {
      Operand->type.RegistersInvlovedInMemory = BXandSI;
      return;
    }
    if (isSubStringExist("BX", Operand->value) &&
        isSubStringExist("DI", Operand->value)) {
...
String rmTosreg(Operand op1, Operand op2) {
  ...
  strcpy(rm, op2.type.OperandType == registerType ? getRegisterCode(op2.value)
                                                  : getRMcode(op2));
...
String sregTorm(Operand op1, Operand op2) {
  ...
  rm = op1.type.OperandType == registerType ? getRegisterCode(op2.value)
                                            : getRMcode(op2);
...
int main(int argc, char **argv) {
  ...
  int OperationType =
      setOperationType(&instruction.operand1, &instruction.operand2);
  switch (OperationType) {
  case regToReg:
    printf("%lX", strtol(regToreg(instruction.operand1, instruction.operand2),
                         NULL, 2));
    break;
  case regToMem:
    printf(
        "%lX%s\n",
        strtol(regTomem(instruction.operand1, instruction.operand2), NULL, 2),
        instruction.operand1.type.Disp);
    break;
  case memToReg:
    printf(
        "%lX%s\n",
        strtol(memToreg(instruction.operand1, instruction.operand2), NULL, 2),
        instruction.operand2.type.Disp);
    break;
  case ImmToRegM:
    printf(
        "%lX%s%s\n",
        strtol(immToRegM(instruction.operand1, instruction.operand2), NULL, 2),
        instruction.operand2.type.dataOrDisp, instruction.operand1.type.Disp);
    break;
  case ImmToReg:
    printf(
        "%lX%s\n",
        strtol(immToReg(instruction.operand1, instruction.operand2), NULL, 2),
        instruction.operand2.type.dataOrDisp);
    break;
  case MemToAcc:
    printf("%lX%s\n", strtol(memToAcc(instruction.operand1), NULL, 2),
           instruction.operand2.type.Disp);
    break;
  case AccToMem:
    printf("%lX%s\n", strtol(accToMem(instruction.operand2), NULL, 2),
           instruction.operand1.type.Disp);
    break;
  case rmToSreg:
    printf("%s%s\n", rmTosreg(instruction.operand1, instruction.operand2),
           instruction.operand2.type.Disp);
    break;
  case SregTorm:
    printf(
        "%lX%s\n",
        strtol(sregTorm(instruction.operand1, instruction.operand2), NULL, 2),
        instruction.operand1.type.Disp);
    break;
  default:
    break;
